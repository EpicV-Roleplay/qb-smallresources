            ...
            CreateThread(function()
                while CruisedSpeed > 0 and IsInVehicle() == Player do
                    ...
                    if not IsTurningOrHandBraking() and GetVehicleSpeed() <
                        ...
                        CruisedSpeed = 0
                        TriggerEvent('seatbelt:client:ToggleCruise')
                        QBCore.Functions.Notify("Tempomat deaktiviert", "error")
                        Wait(2000)
                        break
                    end
...
RegisterCommand('togglecruise', function()
    ...
    if IsDriver() then
        if vehicleClasses[vehClass] then
            ...
            QBCore.Functions.Notify("Tempomat nicht verfügbar", "error")
        end
    end
end, false)

RegisterKeyMapping('togglecruise', 'Tempomat ein- und ausschalten', 'keyboard', 'Y')
